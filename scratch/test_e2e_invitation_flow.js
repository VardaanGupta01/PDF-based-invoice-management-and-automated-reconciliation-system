...
async function testFullFlow() {
  ...
  // 7. Verify Audit Logs recorded INVITATION_SENT and INVITATION_ACCEPTED
  ...
  if (!logActions.includes('INVITATION_SENT') || !logActions.includes('INVITATION_ACCEPTED')) {
    throw new Error('Audit logs missing INVITATION_SENT or INVITATION_ACCEPTED');
  }
  ...
  // 10. Verify Audit Log recorded MEMBER_LEFT
  res = await fetch(`${BASE_URL}/team/audit-logs?businessId=${businessId}`, {
    headers: { 'Authorization': `Bearer ${tokenA}` }
  });
  data = await res.json();
  ...
  if (!finalLogs.includes('MEMBER_LEFT')) {
    throw new Error('Audit logs missing MEMBER_LEFT action');
  }
  ...
  // Verify INVITATION_DECLINED in audit logs
  res = await fetch(`${BASE_URL}/team/audit-logs?businessId=${businessId}`, {
    headers: { 'Authorization': `Bearer ${tokenA}` }
  });
  data = await res.json();
  ...
  if (!declineLogs.includes('INVITATION_DECLINED')) {
    throw new Error('Audit logs missing INVITATION_DECLINED action');
  }
